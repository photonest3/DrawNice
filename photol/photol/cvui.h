...
/**
 ...
 \sa context()
*/
void init(const cv::String& theWindowName, int theDelayWaitKey, bool theCreateNamedWindow, int flag);
...
/**
 ...
 \sa context()
*/
void init(const cv::String theWindowNames[], size_t theHowManyWindows, int theDelayWaitKey, bool theCreateNamedWindows, int flag);
...
/**
 ...
 \sa context()
*/
void watch(const cv::String& theWindowName, bool theCreateNamedWindow, int flag);
...
const int UP = 6;
const int IS_DOWN = 7;
const int WELL_UP = 10;
const int WELL_DOWN = 11;
...
namespace internal
{
	bool isMouseButton(cvui_mouse_btn_t& theButton, int theQuery) {
		...
		switch (theQuery) {
			...
			case cvui::WELL_UP:
				aRet = theButton.well_up; break;
			case cvui::WELL_DOWN:
				aRet = theButton.well_down; break;
		}
	...
	void resetMouseButton(cvui_mouse_btn_t& theButton) {
		...
		theButton.well_up = false;
		theButton.well_down = false;
	}
	...
	bool checkbox(cvui_block_t& theBlock, int theX, int theY, const cv::String& theLabel, bool *theState, unsigned int theColor, double theFontScale) {
		...
		if (aMouseIsOver) {
			...
			if (aMouse.anyButton.justReleased && theColor == 0xCECECE) {
				*theState = !(*theState);
			}
		} else {
...
void init(const cv::String& theWindowName, int theDelayWaitKey, bool theCreateNamedWindow, int flag) {
	...
	watch(theWindowName, theCreateNamedWindow, flag);
}
...
void init(const cv::String theWindowNames[], size_t theHowManyWindows, int theDelayWaitKey, bool theCreateNamedWindows, int flag) {
	...
	for (size_t i = 0; i < theHowManyWindows; i++) {
		watch(theWindowNames[i], theCreateNamedWindows, flag);
	}
}
...
void watch(const cv::String& theWindowName, bool theCreateNamedWindow, int flag) {
	...
	if (theCreateNamedWindow) {
		cv::namedWindow(theWindowName, flag);
	}
...
void update(const cv::String& theWindowName) {
	...
	aContext.mouse.anyButton.well_up = false;
	aContext.mouse.anyButton.well_down = false;
...
void handleMouse(int theEvent, int theX, int theY, int theFlags, void* theData) {
	...
	{
		int value = cv::getMouseWheelDelta(theFlags);
		if (value > 0) {
			aContext->mouse.anyButton.well_up = true;
		}
		else {
			aContext->mouse.anyButton.well_down = true;
		}
	}
	else
	{
