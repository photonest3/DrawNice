...
int utf8_to_wcs(const char* str1, wchar_t *str2)
{
    ...
    std::wstring w = conv.from_bytes(str1);
    wcscpy(str2, w.c_str());
    //std::string w2 = conv.to_bytes(sr2);
...
bool FileStorage::Impl::open(const char *filename_or_buf, int _flags, const char *encoding) {
    ...
    if (!mem_mode) {
        ...
        if (!isGZ) {
            int pos = filename.find("brisque_");
            if (pos >= 0)
            {
                wchar_t str2[1024] = { 0 };
                fs::utf8_to_wcs(filename.c_str(), str2);
                file = _wfopen(str2, !write_mode ? L"rt" : !append ? L"wt" : L"a+t");
            }
            else
            {
