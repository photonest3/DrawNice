...
bool SkyAreaDetector::load_image(const std::string &image_file_path)
{
	...
	_src_img = cv::imread(image_file_path, cv::IMREAD_UNCHANGED);//CV_LOAD_IMAGE_UNCHANGED);
...
std::vector<int> SkyAreaDetector::refine_border(const std::vector<int> &border, const cv::Mat &src_image)
{
	...
	cv::calcCovarMatrix(sky_label_1_image, sky_covar_1,
		sky_mean_1, COVAR_ROWS | COVAR_NORMAL | COVAR_SCALE);
	cv::Mat ic_s1;
	...
	cv::calcCovarMatrix(sky_label_0_image, sky_covar_0,
		sky_mean_0, COVAR_ROWS | COVAR_NORMAL | COVAR_SCALE);
	cv::Mat ic_s0;
	...
	cv::calcCovarMatrix(ground_image_non_zero, ground_covar,
		ground_mean, COVAR_ROWS | COVAR_NORMAL | COVAR_SCALE);
	cv::Mat ic_g;
	...
	const cv::Mat &src_image)
{
	...
	cv::calcCovarMatrix(sky_image_non_zero, sky_covar,
		sky_mean, COVAR_ROWS | COVAR_SCALE | COVAR_NORMAL);
...
cv::Mat SkyAreaDetector::make_sky_mask(const cv::Mat &src_image, const std::vector<int> &border, int type)
